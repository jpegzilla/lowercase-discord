...
var discord=require("discord.js"),$jscomp$destructuring$var0=require("./config.json"),prefix=$jscomp$destructuring$var0.prefix,$jscomp$destructuring$var1=require("./commands"),handleUserCommands=$jscomp$destructuring$var1.handleUserCommands,correction=$jscomp$destructuring$var1.correction,client=new discord.Client,https=require("https");client.once("ready",function(){console.log("ready.");setInterval(function(){return https.get("https://lowercase-discord.herokuapp.com/")},3E5)});
...
"x"),e=b.match(c),f="",g=0;d.split("").forEach(function(a){"x"==a?(f+=e[g].toLowerCase(),++g):f+=a});return{said:"**"+a+" said:** "+f,content:f}};a.content.startsWith(prefix)&&c.test(a.content)&&1==correction?(g=k(d,h).content.replace(prefix,"").trim())&&handleUserCommands(g,a):(b=b.test(a.content)?!0:!1,b||(b=a.content.match(e)?!0:!1),b?!f.test(a.content)&&c.test(a.content)&&f.test(a.content)&&1==correction&&(e=a.content.match(e)[0],f=a.content.replace(e,""),d=k(d,f).said,a.channel.send(""+d+e,g).then(function(){return a["delete"]()})["catch"](function(a){return console.log(a)})):
c.test(a.content)&&1==correction?(console.log(correction),d=k(d,h).said,a.channel.send(""+d,g).then(function(){return a["delete"]()})["catch"](function(a){return console.log(a)})):a.content.startsWith(prefix)&&(g=a.content.replace(prefix,"").trim())&&handleUserCommands(g,a))}});client.login(process.env.TOKEN);var express=require("express"),app=express(),port=process.env.PORT||3E3;app.listen(port,function(){console.log("listening on port",port)});
