...
"").trim())&&handleUserCommands(c,a):(b=b.test(a.content)?!0:!1,b||(b=c.match(a.content)?!0:!1),b||(d.test(a.content)?(c=g(e,f).said,a.channel.send(""+c),a["delete"]()):a.content.startsWith(prefix)&&(c=a.content.replace(prefix,"").trim())&&handleUserCommands(c,a)))}});client.login(process.env.TOKEN);var express=require("express"),app=express(),port=process.env.PORT||3E3;app.listen(port,function(){console.log("listening on port",port)});
