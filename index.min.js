...
client.on("message",function(a){var c=/[A-HJ-Z]|I(?:[A-Za-z0-9])/gm,b=a.member.user.username.toLowerCase();console.log(b);var e=a.content,d=function(a,b){var e=b.match(c),d="";b.split("").forEach(function(a){d=0<=e.indexOf(a)?d+a.toLowerCase():d+a});return{said:"**"+a+" said:** "+d,content:d}};a.content.startsWith(prefix)&&c.test(a.content)?(b=d(b,e).content.replace(prefix,"").trim())&&handleUserCommands(b,a):c.test(a.content)?(b=d(b,e).said,a.channel.send(""+b),a["delete"]()):a.content.startsWith(prefix)&&
(b=a.content.replace(prefix,"").trim())&&handleUserCommands(b,a)});client.login(process.env.TOKEN);var express=require("express"),app=express(),port=process.env.PORT||3E3;app.listen(port,function(){console.log("listening on port",port)});
