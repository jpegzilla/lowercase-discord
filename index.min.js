...
client.on("message",function(a){var b=require("./commands").correction,c=/[A-HJ-Z]|I(?=[A-Za-z0-9])/gm,d=/:(?:[a-zA-Z0-9]+):/gm,g=/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)/gm,e=/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)$/gm,f=a.member.user.username.toLowerCase();if("lowercase"!=f){var h=a.content,k=0<Array.from(a.attachments).length?{files:[Array.from(a.attachments)[0][1].url]}:
...
f+b,k).then(function(){return a["delete"]()})["catch"](function(a){return console.log(a)})):c.test(a.content)&&1==b?(f=l(f,h).said,a.channel.send(""+f,k).then(function(){return a["delete"]()})["catch"](function(a){return console.log(a)})):a.content.startsWith(prefix)&&(k=a.content.replace(prefix,"").trim())&&handleUserCommands(k,a))}});client.login(process.env.TOKEN);var express=require("express"),app=express(),port=process.env.PORT||3E3;
