...
$jscomp.polyfill("String.prototype.startsWith",function(a){return a?a:function(a,b){var c=$jscomp.checkStringArgs(this,a,"startsWith");a+="";for(var d=c.length,g=a.length,h=Math.max(0,Math.min(b|0,c.length)),f=0;f<g&&h<d;)if(c[h++]!=a[f++])return!1;return f>=g}},"es6-impl","es3");var discord=require("discord.js"),$jscomp$destructuring$var0=require("./config.json"),prefix=$jscomp$destructuring$var0.prefix,handleUserCommands=require("./commands"),client=new discord.Client;client.once("ready",function(){console.log("ready.")});
client.on("message",function(a){var c=/[A-HJ-Z]|I(?:[A-Za-z0-9])/gm,b=a.member.user.username,e=a.content,d=function(a,b){var e=b.match(c),d="";b.split("").forEach(function(a){d=0<=e.indexOf(a)?d+a.toLowerCase():d+a});return{said:"**"+a+" said:** "+d,content:d}};a.content.startsWith(prefix)&&c.test(a.content)?(console.log("message caught. running command"),b=d(b,e).content.replace(prefix,"").trim(),console.log("[lowercase] fixed message.",b),a.channel.send("`[lowercase]` command recieved \u21d2 "+
b),b&&handleUserCommands(b,a)):c.test(a.content)?(b=d(b,e).said,a.channel.send(""+b),a["delete"]()):a.content.startsWith(prefix)&&(b=a.content.replace(prefix,"").trim())&&handleUserCommands(b,a)});client.login(process.env.TOKEN);var express=require("express"),app=express();app.listen(process.env.PORT||3E3,function(){console.log("listening on port",process.env.PORT)});
